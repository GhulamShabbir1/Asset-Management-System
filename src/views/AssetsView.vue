...
const assets = [
  ...
  { id: 'AST-44290', category: 'Furniture', model: 'Herman Miller Aeron (Size B)', status: 'MAINTENANCE', statusColor: 'error', statusBg: 'red-lighten-5', custodian: null },
  { id: 'AST-77103', category: 'IT Hardware', model: 'iPhone 15 Pro Max 512GB', status: 'ASSIGNED', statusColor: 'blue-darken-2', statusBg: 'blue-lighten-5', custodian: { name: 'Marcus Knight', initials: 'MK', color: 'success' } },
  { id: 'AST-33019', category: 'Kitchenware', model: 'Commercial Espresso Machine', status: 'AVAILABLE', statusColor: 'primary', statusBg: 'primary-light', custodian: null },
]

const filteredAssets = computed(() => {
  if (activeFilter.value === 'All') return assets
  return assets.filter(a => a.status === activeFilter.value.toUpperCase())
})

const summaryCards = [
  { title: 'STOCK ALERTS', value: '12 items', subtitle: 'Below critical threshold', icon: 'mdi-alert-outline', iconColor: 'error', bg: 'bg-red-lighten-5', btnText: 'View Details' },
  { title: 'UPCOMING MAINTENANCE', value: '08 items', subtitle: 'Scheduled for next 7 days', icon: 'mdi-cog-outline', iconColor: 'blue-grey-darken-2', bg: 'bg-blue-grey-lighten-5', btnText: 'Open Schedule' },
...
<template>
  <v-container fluid class="pa-0 mx-auto" style="max-width: 1400px;">
    ...
    <v-card elevation="0" border class="rounded-lg mb-4">
      ...
      <div class="d-flex align-center justify-space-between pa-4 border-b">
        ...
        <div class="d-flex text-right align-center">
          <div class="pr-5 mr-5 border-e">
            <div class="text-medium-emphasis font-weight-bold mb-1" style="font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px;">Total Assets</div>
            <div class="text-h6 font-weight-bold" style="line-height: 1;">1,248</div>
          </div>
          <div>
            <div class="text-medium-emphasis font-weight-bold mb-1" style="font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px;">Utilization</div>
      ...
      <v-table hover>
        <thead>
          <tr>
            <th style="width: 50px;" class="px-4">
              <v-checkbox-btn density="compact" color="primary"></v-checkbox-btn>
            </th>
            <th class="font-weight-bold text-medium-emphasis text-caption" style="text-transform: uppercase;">Asset ID</th>
            <th class="font-weight-bold text-medium-emphasis text-caption" style="text-transform: uppercase;">Category</th>
            <th class="font-weight-bold text-medium-emphasis text-caption" style="text-transform: uppercase;">Model</th>
            <th class="font-weight-bold text-medium-emphasis text-caption" style="text-transform: uppercase;">Status</th>
            <th class="font-weight-bold text-medium-emphasis text-caption" style="text-transform: uppercase;">Custodian</th>
        ...
        <tbody v-if="filteredAssets.length === 0">
          <tr>
            <td colspan="7" class="text-center py-6 text-medium-emphasis text-caption">No assets found for this filter.</td>
          </tr>
        </tbody>
        <tbody v-else>
          <tr v-for="item in filteredAssets" :key="item.id">
            ...
            <td>
              <v-chip size="small" variant="flat" :color="item.statusBg" :class="[`text-${item.statusColor}`, 'font-weight-bold']">
                {{ item.status }}
              </v-chip>
            </td>
            <td>
              <div v-if="item.custodian" class="d-flex align-center">
                <v-avatar size="24" :color="item.custodian.color" class="mr-2 text-white text-caption font-weight-bold">
      ...
      <div class="d-flex justify-space-between align-center pa-3 border-t bg-grey-lighten-4">
        <span class="text-caption text-medium-emphasis font-weight-medium">Showing 1-10 of 1,248 assets</span>
        <div class="d-flex align-center">
          <v-btn icon="mdi-chevron-left" variant="text" size="small" class="mr-1" disabled></v-btn>
          <v-btn variant="flat" color="black" size="x-small" class="font-weight-bold mx-1 rounded-sm" style="min-width: 24px; height: 24px;">1</v-btn>
          <v-btn variant="text" color="black" size="x-small" class="font-weight-bold mx-1 rounded-sm" style="min-width: 24px; height: 24px;">2</v-btn>
          <v-btn variant="text" color="black" size="x-small" class="font-weight-bold mx-1 rounded-sm" style="min-width: 24px; height: 24px;">3</v-btn>
          <span class="text-caption mx-2">...</span>
          <v-btn variant="text" color="black" size="x-small" class="font-weight-bold mx-1 rounded-sm" style="min-width: 24px; height: 24px;">125</v-btn>
    ...
    <v-row>
      <v-col v-for="(card, i) in summaryCards" :key="i" cols="12" md="4">
        <v-card elevation="0" border class="rounded-lg h-100 d-flex flex-column">
          <v-card-text class="pa-4 flex-grow-1">
            <div class="text-medium-emphasis font-weight-bold mb-3" style="font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px;">{{ card.title }}</div>
            <div class="d-flex align-center">
          ...
          <v-divider></v-divider>
          <v-btn block variant="text" color="black" class="text-none font-weight-bold text-caption rounded-0 py-3" style="height: auto;">
            {{ card.btnText }}
          </v-btn>
        </v-card>
      </v-col>
